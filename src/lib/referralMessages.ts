...
const REMINDER_TEMPLATES: Template[] = [
...
function pickTemplate(templates: Template[], seed: string): Template {
  let hash = 0
  for (let i = 0; i < seed.length; i++) hash = (hash * 31 + seed.charCodeAt(i)) >>> 0
  return templates[hash % templates.length]
}

export function getReferralInitialMessage(name: string, clientId: string, agentName?: string | null): string {
  return pickTemplate(INITIAL_TEMPLATES, clientId)(name, agentSignature(agentName))
}

export function getReferralReminderMessage(name: string, clientId: string, agentName?: string | null): string {
  return pickTemplate(REMINDER_TEMPLATES, `${clientId}-r`)(name, agentSignature(agentName))
}
