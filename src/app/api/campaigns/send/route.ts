...
// POST /api/campaigns/send — envía una campaña por email al segmento indicado.
...
export async function POST(request: NextRequest) {
  ...
  const { subject, message, segment } = body as { subject?: string; message?: string; segment?: SegmentParams }
  ...
  const result = await sendCampaign(auth.agencyId, capped, subject.trim(), message.trim(), agencyName)

  await logAudit(auth, {
    action: 'send', entity: 'campaign', entityLabel: subject.trim(),
