...
• Campañas: envío masivo de emails con DISEÑO DE MARCA (no texto plano) a un segmento de clientes.
  CÓMO ENVIAR UNA CAMPAÑA: 1) En "¿A quién?" elige el segmento (por estatus, estado, etiqueta, con/sin WN, o "un cliente específico" buscándolo por nombre). 2) Opcional: usa una "✨ Plantilla rápida" (Inscripción Abierta, Renovación, Plan Dental, Cumpleaños, Reseña Google, Bienvenida) para llenar todo con un clic. 3) Escribe el Asunto y el Mensaje (usa {nombre} y {agente} para personalizar; puedes adjuntar hasta 3 imágenes y agregar un "Botón de acción" con texto y enlace, ej. "Cotiza ahora"). 4) "👁 Vista previa" muestra el correo con tu logo/nombre, colores de marca, el botón y tus datos de contacto en el pie. 5) "Enviar campaña" (máximo 60 correos por envío; solo a clientes con email). También puedes "Guardar borrador". El diseño (logo, colores, contacto) sale de Configuración → Marca y Perfil.
