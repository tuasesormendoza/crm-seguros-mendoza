...
export async function GET() {
  ...
  const [
    ...
    prisma.client.count({ where: { status: 'Cancelado' } }),
    prisma.client.count({ where: { status: 'Pendiente de Pago' } }),

    // Total lives — sum of affiliatesCount (excludes nulls)
    prisma.client.aggregate({ _sum: { affiliatesCount: true } }),
    // How many rows have null affiliatesCount (each counts as 1 life)
    prisma.client.count({ where: { affiliatesCount: null } }),
    ...
    prisma.client.groupBy({
      ...
      where: { insurer: { not: null } },
    }),

    // By state and coverage type
    prisma.client.groupBy({ by: ['state'], _count: { _all: true }, where: { state: { not: null } } }),
    prisma.client.groupBy({ by: ['coverageType'], _count: { _all: true }, where: { coverageType: { not: null } } }),

    // Upcoming renewals — only needed fields, already filtered in DB
    prisma.client.findMany({
      where: { renewalDate: { gte: today, lte: in60 } },
    ...
    prisma.client.findMany({
      where: { contractDate: { gte: thirtyDaysAgo }, firstPaymentPaid: { not: true } },
      select: { id: true, fullName: true, contractDate: true, firstPaymentPaid: true },
    }),

    // Pending first payment (last 60 days)
    prisma.client.findMany({
      where: { contractDate: { gte: sixtyDaysAgo }, firstPaymentPaid: { not: true } },
